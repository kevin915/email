...
final class Email
{
    ...
    {
        //发送多人时接收的$to参数为数组，发送单人时接收的$to参数为字符串
        $email_to = is_array($to) ? implode(';', $to) : $to;
        ...
        if (is_array($to)) { //发送多人时
            foreach ($to as $val) {
                fputs($fp, "RCPT TO: <" . preg_replace("/.*\\<(.+?)\\>.*/", "\\1", $val) . ">\r\n");
                $lastmessage = fgets($fp, 512);
                if (substr($lastmessage, 0, 3) != 250) {
                    fputs($fp, "RCPT TO: <" . preg_replace("/.*\\<(.+?)\\>.*/", "\\1", $val) . ">\r\n");
                    $lastmessage = fgets($fp, 512);
    ...
    {
        $message = preg_replace("/href\\=\"(?!http\\:\\/\\/)(.+?)\"/i", "href=\"" . '123' . "\\1\"", $message);
        $tmp = "";
        $tmp .= "<html><head>";
        $tmp .= "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=" . config('email_charset') . "\">";
    ...
    {
        $email_to = preg_match("/^(.+?) \\<(.+?)\\>\$/", $email_to,
            $mats) ? $this->email_user ? "=?" . config('email_charset') . "?B?" . base64_encode($mats[1]) . ("?= <" . $mats['2'] . ">") : $mats[2] : $email_to;
        return $email_to;
    }

    private function subject($subject)
    {
        $subject = "=?" . config('email_charset') . "?B?" . base64_encode(preg_replace("/[\r|\n]/", "",
    ...
    {
        if ($from == "") {
            $from = "=?" . config('email_charset') . "?B?" . base64_encode($this->site_name) . "?= <" . $this->email_from . ">";
        } else {
            $from = preg_match("/^(.+?) \\<(.+?)\\>\$/", $from,
                $mats) ? "=?" . config('email_charset') . "?B?" . base64_encode($mats[1]) . ("?= <" . $mats['2'] . ">") : $from;
        }
        $message = "";
        $header = "From: " . $from . "{$this->email_delimiter}";
        $header .= "Reply-to: {$this->email_reply_to}{$this->email_delimiter}";
        ...
        $header .= "charset=" . config('email_charset') . "{$this->email_delimiter}";
        $header .= "Content-Transfer-Encoding: base64" . $this->email_delimiter;
        $header .= "Message-ID: <" . gmdate("YmdHs") . "." . substr(md5($message . microtime()), 0, 6) . rand(100000,
                999999) . "@" . $_SERVER['HTTP_HOST'] . (">" . $this->email_delimiter);
        return $header;
    }
